...
void player_update(PlayerState *p, const Cell *cells,
    ...
    p->grounded = box_solid_ex(cells, p->x, p->y + 1.0f, CHAR_W, CHAR_H, include_plat);
    ...
    if (do_fall && p->grounded && include_plat) {
        ...
        for (int fx = (int)p->x; fx <= (int)p->x + CHAR_W - 1 && !on_plat; fx++)
            if (fx >= 0 && fx < WORLD_W && fy >= 0 && fy < WORLD_H)
                if (CELL_TYPE(cells[fy * WORLD_W + fx].type) == CELL_PLATFORM) on_plat = 1;
    ...
    if (p->vx != 0.0f) {
        ...
        if (!box_solid_ex(cells, new_x, p->y, CHAR_W, CHAR_H, 0)) {
            p->x = new_x;
        } else if (p->grounded) {
            for (int s = 1; s <= MAX_STEP_UP; s++) {
                if (!box_solid_ex(cells, new_x, p->y - (float)s, CHAR_W, CHAR_H, 0)) {
                    p->x = new_x;
                    p->y -= (float)s;
                    break;
                }
    ...
    if (p->vy != 0.0f) {
        ...
        if (!box_solid_ex(cells, p->x, new_y, CHAR_W, CHAR_H, plat_solid)) {
            p->y = new_y;
        } else {
            if (p->vy > 0.0f) {
                float fy = (float)(int)p->y;
                while (!box_solid_ex(cells, p->x, fy + 1.0f, CHAR_W, CHAR_H, plat_solid))
                    fy += 1.0f;
                p->y = fy;
                p->grounded = 1;
            }
